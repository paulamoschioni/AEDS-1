...
int main()
{
    ...
    cin>>descAux;
    prod[i].setDescricao(descAux);
    cout<<"Digite o preco do produto: ";
    cin>>precoAux;
    prod[i].setPreco(precoAux);
    cout<<"Digite o imposto do produto: ";
    cin>>impAux;
    prod[i].setImposto(impAux);
    ...
    cin>>tipoAux;
     ...
     if (tipoAux == 1){
        prod[i].setTipo(t1); // Aqui vc deve mandar por parametro um objeto do tipo Tipo
    } else if (tipoAux == 2){
    prod[i].setTipo(t2);
    }
    ...
    cout<<"O preco final do produto e: R$"<<prod[i].precoFinal()<<endl;
    acumu+=prod[i].precoFinal();
    if(prod[i].precoFinal() > 40)  mais40++;
    }

    float media = acumu / 5;
    cout<<"Media dos produtos: "<<media<<endl;;
    cout<<"Quantidade de produtos acima de 40,00: "<<mais40;
    return 0;
}
